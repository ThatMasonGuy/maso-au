<template>
    <div class="w-full lg:grid lg:min-h-screen lg:grid-cols-2 h-screen">
        <div class="flex items-center justify-center py-8 px-4 mx-auto">
            <div class="mx-auto grid w-[350px] gap-6">
                ...
                <div class="grid gap-2 text-center">
                    <h1 class="text-3xl font-bold mb-6">Welcome, {{ user.firstName || 'Guest' }}</h1>
                    <p class="text-balance text-muted-foreground">
                        Enter your details below to create your account
                    </p>
                </div>
                <form class="grid gap-4 cursor-not-allowed" @submit.prevent="handleAdditionalInfoSubmit">
                    <div class="grid gap-2 pointer-events-none">
                        <Label for="userName">Username</Label>
                        <Input id="userName" type="text" class="pointer-events-none" placeholder="AwesomeUser24" required
                            v-model="userName">{{ userName }}</Input>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="grid gap-2">
                            <Label for="firstName">First Name</Label>
                            <Input id="firstName" type="text" autocomplete="given-name" placeholder="John" required
                                v-model="firstName">{{ firstName }}</Input>
                        </div>
                        <div class="grid gap-2">
                            <Label for="lastName">Last Name</Label>
                            <Input id="lastName" type="text" autocomplete="family-name" placeholder="Appleseed" required
                                v-model="lastName">{{ lastName }}</Input>
                        </div>
                    </div>
                    <div class="grid gap-2">
                        <Label for="emailAddress">Email</Label>
                        <Input id="emailAddress" type="email" autocomplete="email" placeholder="JohnAppleseed@email.com"
                            required v-model="emailAddress">{{ emailAddress }}</Input>
                    </div>
                    <div class="grid gap-2">
                    ...
                    <div class="grid gap-2">
                        ...
                        <div class="relative">
                            <span class="absolute left-3 top-[18px] transform -translate-y-1/2 text-gray-500 text-sm">{{
                    callCode || '+61' }}</span>
                            <Input id="phoneNumber" type="tel" autocomplete="tel" placeholder="0412 345 678" required
                                v-model="phoneNumber" :class="phonePadding">{{ phoneNumber }}</Input>
                        </div>
                    </div>
...
import { createUserDocumentWithAdditionalInfo } from '@/auth';
...
const emailAddress = ref(user.value?.emailAddress || '');
const phoneNumber = ref(user.value?.phoneNumber || '');
const selectedCountry = ref(user.country);
...
const labelSelectedCountry = computed(() => {
    const country = countries.value.find(c => c.code === selectedCountry.value);
  ...
  if (currentUser) {
    try {
      const additionalInfo = {
        ...
        country: labelSelectedCountry.value,
        createdAt: new Date(),
        updatedAt: new Date(),
      };
      const updatedUserData = await createUserDocumentWithAdditionalInfo(currentUser, additionalInfo);
      store.commit('SET_USER', updatedUserData);
